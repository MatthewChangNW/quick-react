  ...
  return (
    <div className='flex flex-row gap-2 flex-wrap'>
      {filteredCourses.map(course => 
        <div className={`flex flex-col border w-48 p-4 rounded-md justify-between ${selected.includes(course.term + String(course.number))? 'border-black' : 'border'}`} key={course.title} onClick={() => toggleSelected(course.term + String(course.number))}>
          <div>
